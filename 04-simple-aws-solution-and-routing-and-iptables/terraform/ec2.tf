...
resource "aws_instance" "inspect" {
  ...
  #user_data_base64 = base64encode(templatefile("inspect-user-data.sh.tpl", {
  user_data = templatefile("inspect-user-data.sh.tpl", {
...
## This time the bastion is next to 'inspect'
resource "aws_instance" "bastion" {
  ...
  user_data = <<-EOF
              #!/bin/bash
              sudo dnf install -y amazon-ssm-agent squid
              sudo systemctl enable --now amazon-ssm-agent
              sudo systemctl enable --now squid
              sudo tee /etc/squid/squid.conf > /dev/null << 'EOTP'
              http_port 3128
